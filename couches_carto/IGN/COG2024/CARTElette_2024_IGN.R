...
fr_initial <- st_read("../sources_shp/data_IGN/COG2024/ADMIN-EXPRESS-COG-CARTO_3-2__SHP_WGS84G_FRA_2024-02-22/ADMIN-EXPRESS-COG-CARTO/1_DONNEES_LIVRAISON_2024-03-00171/ADECOGC_3-2_SHP_WGS84G_FRA-ED2024-02-22/COMMUNE.shp", stringsAsFactors = F)
...
rm(fr_metro, fr_initial, fr_971, fr_972, fr_973, fr_974, fr_976)

# Si tout est OK, on continue
# Doit être inférieur à 100Mb... N'est pas le cas depuis 2021 : 145Mb
print(object.size(fr_total), units = "auto")
COMMUNES_NONSIMP <- fr_total %>% ms_simplify(keep=0.5, keep_shapes=TRUE, sys=TRUE)
# Si sys non mentionné : v8 FATAL ERROR in Reached heap limit : Allocation falied - JavaScript heap out of memory C stack trace
# Galère d'installation nouvelle version de node, puis de mapshaper
# https://stackoverflow.com/questions/69266477/process-out-of-memory-using-ms-simplify-in-r
...
COMMUNES <- COMMUNES_NONSIMP %>% ms_simplify(keep=0.01, keep_shapes=TRUE, sys=TRUE)
...
liste_mailles <- c("DEP","EPCI","ARR" ,"CANOV","ZE2020","UU2020","AAV2020","BV2022","REG")
