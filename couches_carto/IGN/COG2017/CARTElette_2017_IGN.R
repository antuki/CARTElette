...
url <- "http://wxs-telechargement.ign.fr/x02uy2aiwjo9bm8ce5plwqmr/telechargement/prepackage/ADMINEXPRESS-COG-PACK_2017-07-07$ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19/file/ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19.7z"
dest <- "ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19.7z"
######################################################################
...
# dézipper
#system('"C:/Program Files (x86)/7-Zip/7z.exe" x -osource source/ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19.7z')
...
fr_metro <- st_read("./source/ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19/ADMIN-EXPRESS-COG/1_DONNEES_LIVRAISON_2017-06-19/ADE-COG_1-0_SHP_LAMB93_FR/COMMUNE_CARTO.shp",stringsAsFactors = F) %>%
  st_transform(4326) %>%  as('Spatial') 

fr_971 <- st_read("./source/ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19/ADMIN-EXPRESS-COG/1_DONNEES_LIVRAISON_2017-06-19/ADE-COG_1-0_SHP_UTM20W84GUAD_D971/COMMUNE_CARTO.shp",stringsAsFactors = F)%>%
  st_transform(4326) %>% as('Spatial') %>% 
  transformation_shp(rot=0, scale=1, shift=c(-2,41)) 
proj4string(fr_971) <- proj4string(fr_metro)

fr_972 <- st_read("./source/ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19/ADMIN-EXPRESS-COG/1_DONNEES_LIVRAISON_2017-06-19/ADE-COG_1-0_SHP_UTM20W84MART_D972/COMMUNE_CARTO.shp",stringsAsFactors = F)%>%
  st_transform(4326) %>% as('Spatial') %>% 
  transformation_shp(rot=0, scale=1, shift=c(0,41)) 
proj4string(fr_972) <- proj4string(fr_metro)

fr_973 <- st_read("./source/ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19/ADMIN-EXPRESS-COG/1_DONNEES_LIVRAISON_2017-06-19/ADE-COG_1-0_SHP_UTM22RGFG95_D973/COMMUNE_CARTO.shp",stringsAsFactors = F) %>%
  st_transform(4326) %>% as('Spatial') %>% 
  transformation_shp(rot=0, scale=1, shift=c(2,41)) 
proj4string(fr_973) <- proj4string(fr_metro)

fr_974 <- st_read("./source/ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19/ADMIN-EXPRESS-COG/1_DONNEES_LIVRAISON_2017-06-19/ADE-COG_1-0_SHP_RGR92UTM40S_D974/COMMUNE_CARTO.shp",stringsAsFactors = F) %>%
  st_transform(4326) %>% as('Spatial') %>% 
  transformation_shp(rot=0, scale=1, shift=c(4,41)) 
proj4string(fr_974) <- proj4string(fr_metro)

fr_976 <- st_read("./source/ADMIN-EXPRESS-COG_1-0__SHP__FRA_2017-06-19/ADMIN-EXPRESS-COG/1_DONNEES_LIVRAISON_2017-06-19/ADE-COG_1-0_SHP_RGM04UTM38S_D976/COMMUNE_CARTO.shp",stringsAsFactors = F) %>%
  st_transform(4326) %>% as('Spatial') %>% 
